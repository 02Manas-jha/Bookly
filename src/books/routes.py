...
from src.books.schemas import BookSchema ,BookUpdateModel, BookCreateModel, BookDetailModel
...
@book_router.patch("/{book_uid}", response_model=BookSchema, dependencies=[role_checker])
async def update_book(book_uid: str,book_update_data: BookUpdateModel,session: AsyncSession = Depends(get_session),token_details=Depends(access_token_bearer)) -> dict:
